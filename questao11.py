...
def sts_cusum(sim, d):
    ...
    # ══════════════════════════════════════════════════════
    # FASE A: Testar normalidade dos Cᵢ (passos 1-5)
    # ══════════════════════════════════════════════════════
    print(f"        [Fase A] Testando normalidade dos Cᵢ (CUSUM)...")
    while True:
        N = B * M
        total = d + N
        ...
        # Redimensiona para matriz (M blocos, B colunas)
        matriz_blocos = dados.reshape(M, B)
        
        # Média de cada bloco (vetor de tamanho M)
        medias_blocos = matriz_blocos.mean(axis=1, keepdims=True)
        
        # Centraliza os dados de cada bloco em relação à sua própria média
        desvios = matriz_blocos - medias_blocos
        
        # Soma cumulativa ao longo das colunas (dentro de cada bloco)
        S = np.cumsum(desvios, axis=1)
        
        # Calcula Cᵢ: para cada bloco, achamos o argumento que maximiza |S_k|
        # Multiplicamos pelo fator de escala sqrt(12 / (B * (B^2 - 1))) conforme literatura STS
        # Porém, para o teste de normalidade, a escala constante não altera o p-valor do Shapiro-Wilk.
        # Computamos o C_i bruto para o teste:
        C = np.max(np.abs(S), axis=1)

        # Shapiro-Wilk (limitado a 5000 pela scipy)
        amostra_sw = C[:5000] if M > 5000 else C
    ...
    while True:
        ...
        matriz_blocos = dados.reshape(M, B_fixo)
        medias_blocos = matriz_blocos.mean(axis=1, keepdims=True)
        desvios = matriz_blocos - medias_blocos
        S = np.cumsum(desvios, axis=1)
        
        # Estatística C_i baseada no pico do CUSUM do bloco
        C = np.max(np.abs(S), axis=1)
        
        # Estimador de Variância STS/CUSUM para a média global:
        # Var(Xbar) = [ 12 / ( M^2 * B * (B^2 - 1) ) ] * sum( C_i^2 )
        fator_escala = 12.0 / ( (M**2) * B_fixo * (B_fixo**2 - 1) )
        var_xbar = fator_escala * np.sum(C**2)
...
ax.set_title(
    ...
    "Transiente: MSER-5Y  |  Estimação: STS/CSUM  |  "
...
plt.tight_layout()
plt.savefig("./ex11_mm1_sts_csum.png", dpi=160)
plt.close()
print("\nGráfico salvo em: ex11_mm1_sts_csum.png")
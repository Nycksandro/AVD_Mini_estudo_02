...
# Janela de Conway: quantos clientes consecutivos devem passar sem novos máximos/mínimos
JANELA_CONWAY = 150

CSV_RESUMO   = os.path.join(os.path.dirname(os.path.abspath(__file__)), "resultados_resumo5_Conway.csv")
...
for iteracao in range(1, NUM_REPETICAO+1): # Pra executar 30 vezes
    ...
    # Controles da Heurística de Conway por Extremos
    transiente_eliminado = False
    clientes_descartados = 0
    
    max_wq = float('-inf')
    min_wq = float('inf')
    clientes_sem_novos_extremos = 0
    ...
    while id_saida_valida < NUM_CLIENTES_VALIDOS:
        ...
        if proxima_chegada <= proxima_saida:
            ...
            if not servidor_ocupado:
                ...
                # Avaliação de Conway para o cliente atual
                if not transiente_eliminado:
                    if wq > max_wq or wq < min_wq:
                        # Se quebrou o recorde, atualiza os extremos e reseta a contagem
                        max_wq = max(max_wq, wq)
                        min_wq = min(min_wq, wq)
                        clientes_sem_novos_extremos = 0
                    else:
                        clientes_sem_novos_extremos += 1
                    
                    if clientes_sem_novos_extremos >= JANELA_CONWAY:
                        transiente_eliminado = True
                    else:
                        clientes_descartados += 1
                
                # Se o transiente já acabou (ou acabou de acabar neste passo)

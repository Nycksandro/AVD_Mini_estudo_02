...
TAXA_ENTRADA = 9    #  chegadas por segundo
TAXA_SERVICO = 10   #  atendimentos por segundo

CSV_RESUMO = "resultados_resumo.csv"    # uma linha por rodada com os valores estatísticos
...
for n_alvo in NUM_CLIENTES:
    ...
    tempos_espera    = []  
    fila_chegada     = []   # timestamps de chegada em ordem FIFO
    ...
    # simulação (n_alvo) eh para cada ordem de grandeza
    while id_saida <= n_alvo:
        ...
        if proxima_chegada <= proxima_saida:
            ...
            if fila > 0:
                ...
                tempo_espera_fila = tempo_atual - t_chegada_proximo
    ...
    variancia = soma_sq / (n - 1)
    desvio    = math.sqrt(variancia)

    Z         = 1.96
    erro_pad  = desvio / math.sqrt(n)
    margem    = Z * erro_pad
...
print("\n" + "─" * 60)
print(f"  CSV gerado: {CSV_RESUMO}  — resumo estatístico por rodada")
print("─" * 60)